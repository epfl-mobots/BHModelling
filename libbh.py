...
def make_wall(x, y, z, color="saddlebrown", opacity=0.5, name="Wall", showlegend:bool=False):
    return go.Surface(
        x=x, y=y, z=z,
...
class ABC(Frame):
    ...
    def draw_plotly(self, hive_height, hive_depth, color:str, show_legend:bool=False):
        # Create subframes
        frame = []
...
class BoxHive(PlotlyObject):
    ...
    def add_frame(self, position:int, opacity, show_legend:bool = True):
        """
        ...
        frame.draw_plotly(self.height, self.depth, "gold", f"frame_{position}", show_legend)
        # And store it in occupancy
        self.occupancy[position] = frame

    def add_abc(self, position:int, show_legend:bool = True):
        """
        Adds an ABC frame into the hive at the given position.
        """
        # First check that position is between 1 and 10:
        assert position >=1 and position <=10, "Position must be an integer comprised between [0,10]"
        # Then check that the space is free
        ...
        abc = ABC(_x)
        abc.draw_plotly(self.height, self.depth, "red", show_legend)
    ...
    def draw_plotly(self,show_legend:bool=False):
        ...
        # Front wall (y = 1 plane)  <-- swapped
        front_wall = make_wall(
            x=[[0, self.width], [0, self.width]],
            y=[[0, 0], [0, 0]],
            z=[[1, 1], [self.height, self.height]],
            color="peru",
            opacity=self.opacity,
            name="Front",
            showlegend = show_legend
        ...
        # Back wall (y = hive_depth plane)  <-- swapped
        back_wall = make_wall(
            x=[[0, self.width], [0, self.width]],
        ...
        # Left wall (x = hive_width plane)  <-- swapped
        left_wall = make_wall(
